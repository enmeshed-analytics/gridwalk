...
export function ClientWorkspaceDropdown({
  ...
  const handleCreateWorkspace = async (name: string): Promise<void> => {
    try {
      ...
      if (newWorkspace) {
        ...
        router.push(`/${currentSection}?workspace=${newWorkspace.id}`);
      }
    } catch (error) {
  ...
  const handleWorkspaceSelect = async (id: string, name: string) => {
    ...
    try {
      ...
      const currentPage = pathSegments?.[1] || "maps";
      router.push(`/${currentPage}?workspace=${id}`);
    } catch (error) {
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                disabled={isNavigating || workspaces.length === 0}
              >
